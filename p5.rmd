...
# Selects the relevant time period in all three datasets after standardizing date formats
cd %>%
...
Lets take a look at the price of Bitcoin during our selected time period.
...
```
```{r, echo=FALSE, fig.height=10}
ggplot(data=cg, mapping = aes(x = cg$Name, y = cg$`US dollar...2`, color = cg$`US dollar...2`))+
  geom_line()+
  #scale_x_date(date_breaks = "1 Year")+
  ...
  theme_minimal() +
  labs(
    y="Price of Gold in USD",
  x=("Date") ,
  caption = "Price of Gold in USD"
  )
```

```{r, echo=FALSE, fig.height=10}
ggplot(data=cc, mapping = aes(x= cc$Group.1,y =cc$x,color=cc$x)) +
  #geom_line()+
  geom_point()+
  #scale_x_date(date_breaks = "1 Year")+
