...
# Filters the relevant variables and gets cumulative metrics by date by taking the per annum average
cc %>%
  filter(LOCATION=='OECD')
cc <-aggregate(cc$Value, list(cc$TIME), FUN=mean)
cg <- select(cg, c('Name','US dollar...2'))
cc$Group.1 <- as.factor(cc$Group.1)
cc$Group.1 <- as.Date(cc$Group.1, format="%Y-%m")
```
\\
